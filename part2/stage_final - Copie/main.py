...
def bouton_ok():
    global animation, spinbox, data, tab, spinbox, i3, p, indice, monCanvas, ball2, largeur_fenetre, tableau_carte, i_actuelle,boutD, boutSup, boutSuiv, boutF,boutPrec, vitesse, boutAff, boutHeat
    ...
    boutHeat = tkinter.Button(top, text="HeatMap", width = 15, command = heatMap, activebackground="red")
...
def heatMap():
    global tabHeat, data, monCanvas, division, hauteur, largeur, spinbox
    tabHeat = [0] * int(largeur/division)
    for i in range(int(largeur/division)):
        tabHeat[i] = [0] * int(hauteur/division)
    print("taille " + repr(int(largeur/division)) + " " + repr(int(hauteur/division)))
    for i in range (tab[spinbox.get()]["debut"],tab[spinbox.get()]["fin"]-1):
        if(not(is_nan(data["GazePointX (MCSpx)"][i])) and not(is_nan(data["GazePointY (MCSpx)"][i]))):
            x = int(data["GazePointX (MCSpx)"][i]/division)
            y = int(data["GazePointY (MCSpx)"][i]/division)
            if(x < int(largeur/division) and y < int(hauteur/division)):
                print("(" + repr(x) + " , " + repr(y) + ")")
                tabHeat[x][y] +=1
    int_min = tabHeat[0][0]
    int_max = tabHeat[0][0]
    for i in range(int(largeur/division)):
        for j in range(int(hauteur/division)):
            if (tabHeat[i][j] != 0):
                print(tabHeat[i][j])
                monCanvas.create_oval(i, j, i+5, j+5, fill = "green")
                if(int_min > tabHeat[i][j]):
                    int_min = tabHeat[i][j]
                if(int_max < tabHeat[i][j]):
                    int_max = tabHeat[i][j]
    print(int_min)
    print(int_max)
...
def modif_image2(var_img):
    global data, i3, monCanvas, img1, img, ball2, blanc, i2
